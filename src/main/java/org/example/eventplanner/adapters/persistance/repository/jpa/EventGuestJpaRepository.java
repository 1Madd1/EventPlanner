...
import org.example.eventplanner.adapters.persistance.dao.FrequentNoShowersDao;
import org.example.eventplanner.adapters.persistance.dao.Top5GuestsDao;
...
public interface EventGuestJpaRepository extends JpaRepository<EventGuestDao, UUID> {
    List<EventGuestDao> findAllByEventIdAndAttendingTrue(UUID eventId);

    Optional<EventGuestDao> findByEventIdAndGuestId(UUID eventId, UUID guestId);

    List<EventGuestDao> findAllByEventId(UUID eventId);
    ...
    LIMIT 5;
    """, nativeQuery = true)
    List<Top5GuestsDao> findTop5ConfirmedAttendedGuests();
    ...
    """, nativeQuery = true)
    List<FrequentNoShowersDao> findFrequentNoShowers();
